...
是一只**本地运行**的猫娘 AI 助手：网页聊天 + 工具调用（Agent）。<br>
她能读写文件、抓网页、执行 Python 代码、用 Blender 做 3D 建模<br>
（虽然只能做简单模型）（但是有视觉！！！）（算多模态吗、、？）<br>
还有烧token的会议模式。<br>
所有数据只在本地跑<br>
有些文件你也不愿意上传吧zakozako~<br>

桌宠只是随便跑了一个，有想法但是<del>懒得做</del>、后续再说、、<br>
要是有bug或者可以优化的地方请和我说！谢谢喵！
...
- **顺手**：一个网页里就能聊天、传图、切模型、切人格、开讨论；前端是单文件原生 JS，
  改完刷新即生效，没有构建步骤。
- **讨论模式**：拉几个模型互相挑错、辩论，几轮后收敛出一份可执行的结论；
  中途能暂停插话、继续、换个议题重开，结论能一键交给助手去执行。
